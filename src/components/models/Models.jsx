...
function Shoe() {
  ...
  useFrame((state) => {
    ...
    ref.current.rotation.x = -0.2 - (1 + Math.sin(t / 1.5)) / 20
    ref.current.rotation.z = Math.cos(t / 4) / 8
...
function Dolly() {
  ...
  useFrame(({ clock, camera }) => {
    camera.position.x = 50 + Math.sin(clock.getElapsedTime()) * 30
  })
  return null
}
  ...
  return (
    ...
    <Canvas shadows dpr={[1, 2]} camera={{ position: [-120, 0, 0], fov: 55 }}>
        <ambientLight intensity={0.7} />
        <spotLight intensity={0.1} angle={0} penumbra={0.2} position={[10, 15, 10]} castShadow />
        <Suspense fallback={null}>
          ...
          <ContactShadows rotation-x={Math.PI / 2} position={[0, -0.8, 0]} opacity={0.25} width={100} height={10} blur={1.5} far={0.8} />
        </Suspense>
